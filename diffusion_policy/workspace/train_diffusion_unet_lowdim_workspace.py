...
from diffusion_policy.policy.diffusion_unet_lowdim_policy import DiffusionUnetLowdimPolicy # 导入DiffusionUnetLowdimPolicy类
from diffusion_policy.dataset.base_dataset      import BaseLowdimDataset        # 导入BaseLowdimDataset类
from diffusion_policy.env_runner.base_lowdim_runner import BaseLowdimRunner     # 导入BaseLowdimRunner类
...
from diffusers.training_utils                   import EMAModel         # 导入EMAModel类

OmegaConf.register_new_resolver("eval", eval, replace=True) # 注册新的解析器

# %%
class TrainDiffusionUnetLowdimWorkspace(BaseWorkspace): # 定义TrainDiffusionUnetLowdimWorkspace类，继承自BaseWorkspace
    include_keys = ['global_step', 'epoch'] # 包含的键
    # 调用父类 BaseWorkspace 的初始化方法，设置随机种子，实例化模型和优化器，配置训练状态和EMA模型
    ...
    def run(self): # 运行方法
        ...
        lr_scheduler = get_scheduler(   # 获取学习率调度器
            ...
            # pytorch假定每个epoch都步进LRScheduler，然而huggingface diffusers每个batch都步进
        ...
        env_runner: BaseLowdimRunner            # 声明环境运行器类型
        env_runner = hydra.utils.instantiate(   # 实例化环境运行器
            cfg.task.env_runner,
            output_dir=self.output_dir)
        assert isinstance(env_runner, BaseLowdimRunner) # 确认环境运行器类型
            ...
            for local_epoch_idx in range(cfg.training.num_epochs): # 遍历训练轮数
                ...
                if (self.epoch % cfg.training.sample_every) == 0: # 如果到达采样间隔
                    with torch.no_grad(): # 不计算梯度
                        ...
                        batch = train_sampling_batch # 获取训练采样批次
                        obs_dict = {'obs': batch['obs']} # 构建观测字典
                        gt_action = batch['action'] # 获取真实动作
                        
                        result = policy.predict_action(obs_dict) # 预测动作
                        if cfg.pred_action_steps_only: # 如果只预测动作步
                            pred_action = result['action'] # 获取预测动作
                            start = cfg.n_obs_steps - 1 # 获取起始步
                            end = start + cfg.n_action_steps # 获取结束步
                            gt_action = gt_action[:,start:end] # 获取真实动作
                        else: # 如果预测全部动作
...
# 定义主函数，实例化 TrainDiffusionUnetLowdimWorkspace 类并运行
def main(cfg): # 主函数
    workspace = TrainDiffusionUnetLowdimWorkspace(cfg) # 创建TrainDiffusionUnetLowdimWorkspace实例
    workspace.run() # 运行工作区
