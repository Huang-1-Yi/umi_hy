...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        # 改好了 log_with=[]  log_with='wandb' log_with='.。。'
        accelerator = Accelerator(log_with='wandb')# accelerator = Accelerator(log_with='wandb'
        ...
        # resume training 加载指定的检查点。这个方法通常用于将检查点中的模型状态、优化器状态等加载到当前的模型和优化器中，以便可以从之前的训练状态继续训练
        if cfg.training.resume:
            lastest_ckpt_path = self.get_checkpoint_path()
            if lastest_ckpt_path.is_file():
                accelerator.print(f"Resuming from checkpoint!!!!!! {lastest_ckpt_path}")# 返回一个检查点文件的路径
                self.load_checkpoint(path=lastest_ckpt_path)# 检查获取到的检查点路径是否指向一个存在的文件
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0 and accelerator.is_main_process:
                    ...
                    # We can't copy the last checkpoint here 我们无法在此处复制最新的检查点
                    # since save_checkpoint uses threads. 因为 save_checkpoint 使用了线程
                    # therefore at this point the file might have been empty! 因此，在这种情况下，文件可能为空！
