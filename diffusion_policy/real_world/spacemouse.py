...
class Spacemouse(Thread):
    def __init__(self, max_value=500, deadzone=(0,0,0,0,0,0), dtype=np.float32):
        ...
        *----->x right
        ...
        self.button_state = defaultdict(lambda: False, {i: False for i in range(26)})
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if isinstance(event, SpnavMotionEvent):
                    ...
                    # 检查按钮编号是否在预期范围内
                    if event.bnum < len(self.button_state):
                        self.button_state[event.bnum] = event.press
                    else:
                        print(f"Error: event.bnum {event.bnum} is out of bounds.")
                else:
                    time.sleep(1/200)
        finally:
            spnav_close()
