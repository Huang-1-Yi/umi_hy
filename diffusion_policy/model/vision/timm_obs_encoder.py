...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        model = timm.create_model(
            ...
            pretrained=True,
            pretrained_cfg_overlay=dict(file='/home/robot/umi/timm/vit_base_patch16_clip_224.openai/pytorch_model.bin'),
        ...
        # 如果类型是 'rgb'，则观察名称添加到 rgb_keys 列表中，并将模型和变换存储在 key_model_map 和 key_transform_map 字典中。如果 share_rgb_model 为 False，则复制模型，否则使用同一个模型实例。
        # 如果类型是 'low_dim'，则观察名称添加到 low_dim_keys 列表中，除非该观察被策略忽略（ignore_by_policy 为 True）。
        ...
        print(' RGB 观察 rgb keys:         ', rgb_keys)
        print(' 低维 观察 low_dim_keys keys:', low_dim_keys)
        ...
        if model_name.startswith('vit'):
            ...
            if self.feature_aggregation == 'all_tokens':
                ...
                # 对于ViT（视觉Transformer）模型，如果特征聚合方式是 all_tokens，则使用所有token；否则发出警告信息并忽略特征聚合设置，使用CLS token
                logger.warn(f'发出警告信息并忽略特征聚合设置，使用CLS token vit will use the CLS token. feature_aggregation ({self.feature_aggregation}) is ignored!')
    ...
    def forward(self, obs_dict):
        ...
        # 处理RGB输入process rgb input
        # 调整图像的形状并应用相应的变换 key_transform_map。
        # 提取原始特征 raw_feature 并进行特征聚合。
        # 确保特征的形状正确，然后将其添加到 features 列表中
        ...
        for key in self.low_dim_keys:
            ...
            # print("数据的形状data shape:", data.shape)
            # print("预期的形状expected shape:", self.key_shape_map[key])
            # 断言数据的形状与预期的形状匹配，如果不匹配会输出形状不匹配的错误信息
