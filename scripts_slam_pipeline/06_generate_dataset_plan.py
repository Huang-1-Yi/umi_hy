"""
整个脚本是一个数据预处理和分析的复杂流程
目的是将从多个摄像头收集到的视频数据,通过分析视频中的视觉标记(如ArUco标记),提取机器人的操作动作(如夹持器的开合)、摄像头的位置和朝向等信息,然后将这些信息组织成一个结构化的数据集,供后续的机器学习或数据分析使用
通过自动化这一流程,可以显著提高处理大量视频数据的效率,为研究和开发人员提供准确且丰富的数据资源
...
# %%
"""
导入了多个Python标准库和第三方库,包括用于文件和系统路径操作的os、sys和pathlib,用于命令行界面的click,用于数值计算和数据处理的numpy、scipy和pandas,以及用于读取和处理视频文件的av和exiftool等。
"""
import sys
import os
...
# %%
# 接受一个布尔序列,返回这个序列中连续真值或假值段的切片以及这些段的类型（真或假）
...
# 从一个包含时间戳和相机姿态（位置和四元数旋转）的DataFrame中生成姿态插值器
...
# 计算两个坐标系之间在x轴上的投影差异,用于后续的左右夹持器区分
...
# %%
# 定义了一个命令行界面,接受多个参数和选项,用于指定输入输出路径、TCP偏移、SLAM标签路径、名义Z值、最小剧集长度和忽略的摄像头
"""
阶段 0: 准备阶段
解析输入路径,准备输出路径,计算摄像头到夹持器的变换矩阵。
阶段 1: 提取视频元数据
遍历视频目录,提取视频的元数据信息,如摄像头序列号、视频开始时间、帧数等。
阶段 2: 将视频匹配到演示中
通过视频的时间戳信息,将多个摄像头拍摄的视频匹配成一个完整的演示（demo）。
阶段 3: 识别夹持器ID
通过分析视频中的ArUco标记,识别每个视频对应的夹持器硬件ID。
阶段 4: 区分夹持器左右
通过计算每个夹持器相对于其它夹持器在空间中的位置,区分夹持器是左边的还是右边的。
阶段 6: 生成数据集计划
基于前面的分析结果,生成一个包含所有需要处理视频的详细计划,此计划包括每个演示的时间戳、每个夹持器的位姿和开合宽度,以以及每个摄像头的视频路径和视频中有效帧的起止。用于后续的数据处理和分析。
遍历所有演示数据,针对每个演示进行处理,包括：
    确定演示的有效时间段
    加载并处理相关的摄像头姿态数据和夹持器宽度数据
    根据有效时间段和数据生成每个演示的详细信息
"""
@click.command()
@click.option('-i', '--input', required=True, help='Project directory')
@click.option('-o', '--output', default=None)
@click.option('-to', '--tcp_offset', type=float, default=0.205, help="Distance from gripper tip to mounting screw")# 从夹爪尖端到安装螺丝的距离
...
def main(input, output, tcp_offset, tx_slam_tag,
    ...
    # %% stage 0: gather inputs准备阶段:解析输入路径,准备输出路径,计算摄像头到夹持器的变换矩阵
    ...
    # tcp到摄像机的转换，单位为米，相机帧中的y轴
    cam_to_center_height = 0.086        # UMI常数 constant for UMI
    # 当光学中心在支架前面时，光学中心到安装螺钉
    ...
    # SLAM映射原点到表的标记转换 SLAM map origin to table tag transform
    ...
    # 加载夹具校准，并将这些数据存储在字典里，包括夹持器的最小和最大宽度 load gripper calibration
    ...
    # %% stage 1 提取视频元数据:遍历视频目录,提取视频的元数据信息,如摄像头序列号、视频开始时间、帧数等
    ...
    # %% stage 3 ：使用 aruco 识别夹持器 ID
    # 通过分析视频中的 ArUco 标记，识别每个视频对应的夹持器硬件 ID，并将这些信息添加到 video_meta_df 数据框中
    # output: 
    ...
    for vid_idx, row in video_meta_df.iterrows():
        ...
        if tag_stats:
            ...
            print("tag_stats is empty, cannot determine max tag id.",tag_stats)
        tag_per_gripper = 6
    ...
    # %% stage 4：计算每个摄像头在 x 轴上的投影差异，用于区分左右夹持器。最终为每个摄像头分配索引，并将这些信息添加到 video_meta_df 数据框中
    ...
    # %% stage 6：生成数据集计划generate dataset plan
    # output：
    # all_plans = [{
    ...
    # 将生成的所有演示（demo）的计划信息序列化为Pickle文件,以便于后续的数据加载和处理
    pickle.dump(all_plans, output.open('wb'))
    
# 提供了一个测试函数的框架,用于在实际应用之前测试脚本的功能
def test():
    # %%
