...
# 用于处理相机轨迹数据与标签检测数据，并输出一个包含转换矩阵的JSON文件。
...
def main(tag_detection, csv_trajectory, output, tag_id, keyframe_only):
    """
    请使用通过用映射运行产生的 map_atlas.osa 对映射视频进行重新定位（初始化）所生成的 camera_trajectory.csv。这比映射运行本身产生的 mapping_camera_trajectory.csv 要准确得多。
