...
def get_obs_dict(camera, controller, gripper, n_obs_steps, img_tf):
    camera_data = camera.get(k=n_obs_steps*2-1)
    robot_data = controller.get_all_state()
    gripper_data = gripper.get_all_state()

    raw_imgs = camera_data['color'][::2]
    img_timestamps = camera_data['camera_capture_timestamp'][::2]
    imgs = list()
    for x in raw_imgs:
        # bgr to rgb
...
@click.option('-s', '--steps_per_inference', type=int, default=25)
            ...
            while True:
                ...
                try:
                    ...
                    while True:
                        ...
                        gripper_action = action[:,-1] * 1000 # m to mm
