...
class RealmanInterface:
    ...
    # 自编写
    # 获取机器人的末端执行器（EE）的目标姿态————从服务器获取EE的target_pose
    def get_target_ee_pose(self):
        target_ee_pose = np.array(self.server.get_target_ee_pose())
        return target_ee_pose
    
    # 自编写
    # 获取机器人的所有关节的目标位置___从服务器获取关节位置,并将其转换为numpy数组,返回关节位置数组
    def get_target_ee_pose(self):
        return np.array(self.server.get_target_ee_pose())
...
class RealmanInterpolationController(mp.Process):
    ...
    def __init__(   self, shm_manager: SharedMemoryManager,
                    robot_ip = '192.168.1.18',                  # hy设定的中层控制器的IP地址(NUC)
                    robot_port = 8080, 
                    frequency = 1000,                            # 频率:fr=1000 CB2=125, UR3e=500
                    # lookahead_time=0.1, # 前瞻时间：[0.03, 0.2]秒,使用这个前瞻时间平滑轨迹
                    # gain=300,# 增益：[100, 2000] 跟踪目标位置的增益
                    # Kx_scale=1.0, # 位置增益的比例 
                    # Kxd_scale=1.0, # 速度增益的比例 
                    max_pos_speed=0.3,                         # 最大位置速度：米/秒# UR是0.25  5% of max speed  m/s
                    max_rot_speed=0.3,                          # 最大旋转速度：弧度/秒# UR是0.16  5% of max speed rad/s
                    launch_timeout=3,
                    joints_init=None,
                    # joints_init_speed=1.05,
                    joints_init_duration=None, 
                    payload_mass=None,                          # 有效载荷质量：浮点数
                    payload_cog=None,                           # 有效载荷重心：3D位置,重心
                    joints_init_speed=0.2,
                    receive_keys=None,
                    soft_real_time=False,                       # 预设启用软实时调度___启用轮询调度和实时优先级
                    verbose=False, get_max_k=None, receive_latency=0.0):   # 是否接收日志、frequency * 5、接收延迟                  
        if joints_init is not None:                             # 将joints_init转换为一个NumPy数组,该数组的形状是6
    ...
    # ========= command methods——self.input_queue.put(message) ============
    ...
    def run(self):
        ...
        try:                                        # verbose=1,需要显示,打印一条消息,表明控制器正在连接到机器人,并显示机器人的 IP 地址
            ...
            if self.joints_init is not None:# Realman 实际上没用上
                robot.move_to_joint_positions(self.joints_init, self.joints_init_duration, 0)

            dt = 1. / self.frequency
            curr_pose = robot.get_ee_pose()         # print("4.2.2 当前关节角度ActualQ, curr_joint==",curr_pose)
            ...
            while keep_running:
                ...
                pose_command = pose_interp(t_now)               # Realman 方法3 # 计算并返回在当前时间 t_now 下插值得到的姿态
                tag = robot.update_desired_ee_pose(pose_command)# 更新机器人末端执行器的期望位姿
                # print("robot.update_desired_ee_pose(pose_command)",tag)# 关节空间运动到目标位姿————不阻塞也会存在卡顿
                state = dict()                                  # 更新机器人状态,并将其存储在 state 字典中
                for key, func_name in self.receive_keys:        # Realman 方法3：从机器人接收到的不同状态信息,如末端执行器的实际位姿、关节位置和速度
                ...
                for i in range(n_cmd):
                    ...
                    if cmd == Command.STOP.value:
                        ...
                        # 当前的机器人姿态 滞后于期望的目标姿态__机器人的动态响应限制、控制系统延迟、传感器数据处理时延等因素造成
