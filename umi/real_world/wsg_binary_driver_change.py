...
3.def script_query(self):
    return self.custom_script(0xB0)
功能：该方法发送一个自定义脚本查询命令 0xB0，用于获取机械手的当前状态信息。
参数：没有额外参数，直接使用命令 0xB0。
返回值：返回一个字典，包含命令的响应信息。字典中的主要字段有：
state：机械手的当前状态。
position：机械手的当前位置。
velocity：机械手的当前速度。
force_motor：机械手的电机力。
measure_timestamp：测量时间戳。
is_moving：布尔值，指示机械手是否在移动。
返回值示例：
{
    'state': 0,
    'position': 100.0,
    'velocity': 0.0,
    'force_motor': 0.0,
    'measure_timestamp': 1621256789.123,
    'is_moving': False
}



4.def script_position_pd(self, 
                       position: float, velocity: float,
                       kp: float=15.0, kd: float=1e-3,
                       travel_force_limit: float=80.0, 
                       blocked_force_limit: float=None):
    ...
    assert kd >= 0
    return self.custom_script(0xB1, position, velocity, kp, kd, travel_force_limit, blocked_force_limit)
功能：该方法发送一个自定义脚本命令 0xB1，用于设置机械手的位置和速度，并进行比例-微分（PD）控制。
参数：
position：目标位置。
velocity：目标速度。
kp：比例控制系数。
kd：微分控制系数。
travel_force_limit：运动时的力限制。
blocked_force_limit：阻塞时的力限制。
返回值：返回一个字典，包含命令的响应信息。字典中的主要字段有：
state：机械手的当前状态。
position：机械手的当前位置。
velocity：机械手的当前速度。
...
g = zerorpc.Server(WSGBinaryDriver())
g.bind("tcp://127.0.0.1:5556")
g.run()        