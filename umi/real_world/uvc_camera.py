...
# 实现一个多进程控制摄像头和记录视频的框架。具体功能和结构如下：


# 定义用于控制摄像头的命令，包括重新启动队列（RESTART_PUT）、开始录制（START_RECORDING）和停止录制（STOP_RECORDING）
class Command(enum.Enum):
    RESTART_PUT = 0
    START_RECORDING = 1
    STOP_RECORDING = 2

# 继承自 multiprocessing.Process 的类，用于控制摄像头和视频录制
# 提供了多种功能，包括初始化、启动、停止摄像头和视频录制，以及与共享内存队列和环形缓冲区交互的能力
...
class UvcCamera(mp.Process):
    ...
    def __init__(
        ...
        # verbose =True # 强行调试
        # if verbose == True:
        #     print("UvcCamera.dev_video_path == {}".format(dev_video_path)) 
        #     print("UvcCamera.resolution == {}".format(resolution)) 
        #     print("UvcCamera.capture_fps == {}".format(capture_fps))
        #     print("UvcCamera.get_max_k == {}".format(get_max_k))
        #     print("UvcCamera.receive_latency == {}".format(receive_latency))
        #     print("UvcCamera.cap_buffer_size == {}".format(cap_buffer_size))
    ...
    # ========= context manager ===========
    # 得 UvcCamera 类可以作为一个上下文管理器使用。当进入上下文时，会启动摄像头和视频录制；当退出上下文时，会停止摄像头和视频录制。
    def __enter__(self):
        self.start()
        return self
    ...
    def run(self):
        ...
        if not cap.isOpened():
            print("无法打开摄像头,path== {}".format(self.dev_video_path))
            print("无法打开摄像头,num_threads== {}".format(self.num_threads))
            return
        
        # cap.set(cv2.CAP_PROP_FRAME_WIDTH,1920)   # 设置摄像头的宽度为1920像素
        # cap.set(cv2.CAP_PROP_FRAME_HEIGHT,1080)  # 设置摄像头的宽度为1080像素
        # fourcc=cv2.VideoWriter_fourcc(*'DIVX')   # DIVX代表视频编解码器。fourcc 是 cv.VideoWriter 对象的一个参数，用于指定视频编解码器
        # # cap_writer=cv2.VideoWriter('{}.mp4'.format(camera_index),fourcc,60.0,(1920,1080))# 1920,1080   640, 480  # 为每个摄像头录制一个不同的视频文件。fourcc 是视频编解码器，30.0 是帧率，(640, 480) 是视频的尺寸
