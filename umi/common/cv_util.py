...
"""
img: 这是输入的图像，它应该是一个 NumPy 数组，并且形状应该是 (height, width, channels)。
color=(0,0,0): 这是一个元组，定义了遮罩的颜色。在这里，颜色是黑色，因为 (0,0,0) 对应于 RGB 颜色空间中的黑色。
mirror=True: 这是一个布尔值，指示是否绘制镜像对称的形状。
gripper=True: 这是一个布尔值，指示是否绘制机器人夹具的形状。
finger=True: 这是一个布尔值，指示是否绘制机器人的手指形状。
use_aa=False: 这是一个布尔值，指示是否使用抗锯齿（antialiasing）技术。抗锯齿技术可以平滑图像中线条的边缘，减少锯齿效果。
"""
def draw_predefined_mask(img, color=(0,0,0), mirror=True, gripper=True, finger=True, use_aa=False):
    all_coords = list()
    # 如果 mirror 是 True，则调用 get_mirror_canonical_polygon() 函数来获取镜像对称形状的坐标，并将它们添加到 all_coords 中。
    if mirror:
        all_coords.extend(get_mirror_canonical_polygon())
    # 如果 gripper 是 True，则调用 get_gripper_canonical_polygon() 函数来获取机器人夹具的坐标，并将它们添加到 all_coords 中。
    if gripper:
        all_coords.extend(get_gripper_canonical_polygon())
    # 如果 finger 是 True，则调用 get_finger_canonical_polygon() 函数来获取机器人手指的坐标，并将它们添加到 all_coords 中。
    if finger:
        all_coords.extend(get_finger_canonical_polygon())
    # 函数遍历 all_coords 中的每个坐标，将坐标从基准坐标系转换为像素坐标系。这通过调用 canonical_to_pixel_coords() 函数实现    
    for coords in all_coords:
        pts = canonical_to_pixel_coords(coords, img.shape[:2])
        pts = np.round(pts).astype(np.int32)
        flag = cv2.LINE_AA if use_aa else cv2.LINE_8
        # 使用 OpenCV 库的 cv2.fillPoly() 函数绘制多边形
